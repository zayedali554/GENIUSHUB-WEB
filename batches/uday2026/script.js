...
function updateURL(section, subject = '', chapter = '') {
    if (section === 'subjects') {
        // For subjects view: #subjects
        window.history.replaceState(null, null, '#subjects');
    } else if (section === 'chapters' && subject && !chapter) {
        // For chapters view: #subject (no change needed)
        window.history.replaceState(null, null, `#${subject}`);
    } else if (section === 'content' && subject && chapter) {
        // For content view: #subject/chapter
        window.history.replaceState(null, null, `#${subject}/${encodeURIComponent(chapter)}`);
    }
}

function parseURL() {
    const hash = window.location.hash.substring(1); // Remove #
    if (!hash || hash === 'subjects') {
        return { section: 'subjects', subject: '', chapter: '' };
    }

    const parts = hash.split('/');
    if (parts.length === 1 && parts[0] && parts[0] !== 'subjects') {
        // Format: #subject (chapters view)
        return {
            section: 'chapters',
            subject: parts[0],
            chapter: ''
        ...
        // Format: #subject/chapter (content view)
        return {
            section: 'content',
            subject: parts[0],
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function showSubjects(updateUrl = true) {
        ...
        // Check if we're coming from chapters view
        if (chaptersSection.style.display === 'block') {
            ...
            // Direct show (batch mode - no welcome card)
    ...
    // Import chaptersData from id.js
    if (typeof window !== 'undefined' && window.chaptersData) {
        var chaptersData = window.chaptersData;
    } else {
        var chaptersData = {};
    }

    // Update subject cards with chapter counts
    updateSubjectChapterCounts();
    ...
    if (liveClassesCard) {
        liveClassesCard.addEventListener('click', function () {
            window.open('../../Live/player.html?batch=uday2026', '_blank');
        });
    }
    ...
    function showChapters(subject, updateUrl = true) {
        ...
        subjectData.chapters.forEach(async (chapter) => {
            ...
            try {
                const videosArr = await window.youtubeAPI.getChapterVideos(chapter.name, 50);
            ...
            try {
                ...
                const notesData = await window.googleSheetsAPI.getSheetData(subjectKey, chapter.name, 'Notes');
            ...
            try {
                ...
                const dppData = await window.googleSheetsAPI.getSheetData(subjectKey, chapter.name, 'DPP');
    ...
    const googleSheetsAPI = new GoogleSheetsAPI(SHEETS_CONFIG.API_KEY);
    
    // Make API instances globally accessible
                ...
                if (isAllContents) {
                    ...
                    if (subjectKey && chaptersData[subjectKey]) {
                        ...
                        for (const chapter of chapters) {
                            const chapterVideos = await window.youtubeAPI.getChapterVideos(chapter.name, 999);
                            videos = videos.concat(chapterVideos);
                        }
                    }
                } else {
                    // Get ALL videos for specific chapter
                    videos = await window.youtubeAPI.getChapterVideos(chapterName, 999);
                }
    ...
    // Function to open our embedded player.html in a NEW TAB
    window.openYouTubeVideo = function (videoUrl, videoTitle, chapterName, subjectName) {
        ...
        const playerUrl = `../../player.html?v=${videoId}&title=${encodeURIComponent(videoTitle || 'Video Lecture')}&chapter=${encodeURIComponent(chapterName || '')}&subject=${encodeURIComponent(subjectName || '')}`;
    ...
    // Show subjects directly on page load
    showSubjects(false);
    ...
    // Add back to main page functionality
    const backToMainBtn = document.getElementById('back-to-main-btn');
    if (backToMainBtn) {
        backToMainBtn.addEventListener('click', function() {
            // Redirect to main landing page with clean URL (no .html)
            window.location.href = '../../';
        });
    }

    // Header animation removed since header was removed from HTML
    ...
    function openYouTubeVideo(videoUrl, videoTitle, chapterName, subjectName) {
        ...
        const playerUrl = `../../player.html?v=${videoId}&title=${encodeURIComponent(videoTitle)}&chapter=${encodeURIComponent(chapterName)}&subject=${encodeURIComponent(subjectName)}`;
